...
The Investigation Contract that binds every Refactor Radar detection workflow (`rrd-detect-dependencies`, `rrd-detect-instability`, `rrd-detect-data-issues`, `rrd-detect-duplication`, `rrd-detect-complexity`, `rrd-detect-logging`, `rrd-detect-config`, `rrd-detect-locators`, `rrd-detect-layering`, `rrd-analyze-test-reliability`, `rrd-establish-execution-baseline`, `rrd-audit-all`). Stated once here, not repeated per workflow.
...
Fall back to `Read`/`Grep` only when the target genuinely isn't in the graph (non-code config/text files the indexer doesn't model as symbols — e.g. raw `.properties`, `.xml`, `.yaml` files) or when `get_code_snippet`'s ambiguous-name suggestions can't resolve to the exact symbol. Say so explicitly when falling back rather than defaulting to `Read` out of habit. Note: in practice `get_code_snippet` has returned full content for some non-code text files too (e.g. `.properties` files indexed as `Variable` nodes) — the graph's coverage may extend further than expected, so try it before assuming a fallback is needed.
