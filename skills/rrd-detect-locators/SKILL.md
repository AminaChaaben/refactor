---
name: rrd-detect-locators
description: 'Find locator-strategy gaps: duplicated selectors across page objects, absent priority convention, missing centralized element repository. Use when the user says "detect locator issues" or "audit selector strategy"'
---

# Detect Locators

**Goal:** Find gaps in *locator strategy* — duplicated selector definitions, no consistent priority tiering (`data-testid` > ARIA/role > semantic > absolute XPath), and no centralized element repository — as distinct from raw selector fragility, which `rrd-detect-instability` already owns.
