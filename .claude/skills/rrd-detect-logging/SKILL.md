---
name: rrd-detect-logging
description: 'Find diagnosability gaps — error paths, external calls, and loops with no logging signal — that will block future root-cause analysis. Use when the user says "detect logging" or "find missing logging/instrumentation gaps"'
---

# Detect Logging

**Goal:** Find catch blocks, external-call sites, and failure-capable loop bodies that carry no logging signal — or that log but drop the original exception, breaking root-cause traceability — with file:line precision and a concrete logging-statement proposal. This is instrumentation-for-diagnosability, not style: the axis exists so Category 1's log-driven analysis (`rrd-analyze-test-reliability`) has a real signal to point at later, instead of a stack trace and a shrug.
